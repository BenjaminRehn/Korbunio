...
This uses the prebuilt `ghcr.io/lesecuritae/korbunio:latest` image published through GitHub Container Registry. The default setup does not require a `.env` file.
...
The Compose project, service, and container are all named `korbunio`; persistent data is stored in the `korbunio-data` volume.
...
- Watching: `watch_product`, `list_watches`, `remove_watch` (up to 20). Offers only change weekly, so the server looks every few hours (reloading only when the cache has expired) and reports each new match once, optionally only below a maximum price. Messages go to an address entered on `/settings` (an ntfy topic or a webhook, via POST); without it these tools do not exist.
